...
/**
 * Glitter Wrap — Originkit (Variant 1)
 * Exact Framer canvas starfield warp for Expo DOM.
 */
...
const COMPONENT_DEFAULTS = {
  // Variant 1 — white starfield warp (matches Originkit preview image)
  particleCount: 500,
  color1: '#ffffff',
  color2: '#ffffff',
  color3: '#ffffff',
...
export default function GlitterWrap(props: Props) {
  ...
  const colorCacheRef = useRef({
    ...
    parsed2: [245, 245, 245, 1] as [number, number, number, number],
    parsed3: [224, 224, 224, 1] as [number, number, number, number],
  });
  ...
  useEffect(() => {
    ...
    // DOM root fill (same pattern as TornadoVortex)
    ...
    // Warm-up so first paint looks populated (like Framer static export).
